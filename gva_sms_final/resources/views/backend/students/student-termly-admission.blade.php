...
@section('admin_content')
    ...
    <div class="content p-2 row">
        ...
        <div class="col-md-4">
            ...
            <div class="card-header small">
                <h5 class="modal-title" id="admitStudentModalLabel">Daily Report Students (Day-scholars)</h5>
            </div>
            <div class="card-body">
                {{-- Admission Form --}}
                <form action="{{ route('dayscholars.student.termly-report.store') }}" method="POST">
                    ...
                    <div class="form-group col-md-12">
                        <label for="academic_term" class="small">Select Term</label>
                    ...
                    <div class="form-group">
                        <label for="students" class="small">Select Students to Report In/Out</label>
                        <select class="form-control select2 form-control-sm" id="students" name="student_id" required>
                    ...
                    <div class="form-group col-md-12">
                        <label for="report_status" class="small">Reporting Status</label>
                        <select class="form-control form-control-sm" id="report_status" name="report_status" required>
                    ...
                    <div class="form-group col-md-12">
                        <label for="reported_date" class="small">Date Reported</label>
                        <input type="date" class="form-control form-control-sm" id="reported_date" name="reported_date"
                            required>
                    </div>

                    <div class="form-group col-md-12">
                        <label for="clearInOut_guardian" class="small">Clear-Out Guardian</label>
                        <select class="form-control form-control-sm" id="clearInOut_guardian" name="clearInOut_guardian"
                            required>
                            <option value="">--Select--</option>
                            <option value="parent">Parent</option>
                            <option value="other">Other</option>
                        </select>
                        <div class="form-group d-none" id="parent_id_wrapper">
                        ...
                        <div id="other_clearOut_details" class="d-none">
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    <label for="other_name" class="small">Name</label>
                                    <input type="text" class="form-control form-control-sm" id="other_name"
                                        name="other_name" placeholder="Full Name">
                                </div>
                                <div class="form-group col-md-6">
                                    <label for="other_nrc" class="small">NRC</label>
                                    <input type="text" class="form-control form-control-sm" id="other_nrc"
                                        name="other_nrc" placeholder="National Registration Card Number">
                                </div>

                                <div class="form-group col-md-6">
                                    <label for="other_contact" class="small">Contact Number</label>
                                    <input type="text" class="form-control form-control-sm" id="other_contact"
                                        name="other_contact" placeholder="e.g., 097xxxxxxx">
                                </div>
                                <div class="form-group col-md-6">
                                    <label for="vehicle_reg" class="small">Vehicle Registration #</label>
                                    <input type="text" class="form-control form-control-sm" id="vehicle_reg"
                                        name="vehicle_reg" placeholder="Vehicle Registration Number">
                                </div>
                            </div>
                            <div class="form-group col-md-12">
                                <label for="pickup_by_relationship" class="small">Brought By Relationship</label>
                                <select class="form-control form-control-sm" id="pickup_by_relationship"
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            const guardianSelect = $('#clearInOut_guardian');
            const parentWrapper = $('#parent_id_wrapper');
            const parentSelect = $('#parent_id');
            const otherDetails = $('#other_clearOut_details');
            ...
            guardianSelect.change(function() {
                ...
                if (selectedValue === 'parent') {
                    ...
                    // AJAX call to fetch parents
                    $.ajax({
                        url: "{{ route('fetch.parentByStudentId') }}",
                        method: 'GET',
                        data: {
                            student_id: selectedStudentId
                        }, // Pass student_id as a query parameter
                        beforeSend: function() {
                            // Show a loader or disable dropdown before the request is sent
                            parentSelect.prop('disabled', true).empty().append(
                        ...
                        success: function(response) {
                            ...
                            if (response.success && response.parents.length > 0) {
                                ...
                                $.each(response.parents, function(index, parent) {
                                    parentSelect.append(
                                        `<option value="${parent.user_id}">${parent.name} (${parent.contact_number})</option>`
                                    );
                                });

                                // Show the parent dropdown and hide other details
                                parentWrapper.removeClass('d-none');
                                otherDetails.addClass('d-none');
                            } else {
                                alert(response.message || 'No parents found.');
                                parentWrapper.addClass('d-none');
                            }
                        },
                        error: function(xhr, status, error) {
                            console.error('Error fetching parents:', error);
                            alert('Unable to fetch parents. Please try again later.');
                        },
                        complete: function() {
                            // Enable dropdown after request completion
                            parentSelect.prop('disabled', false);
                        },
                    });
                } else if (selectedValue === 'other') {
                    // Hide parent dropdown and show other details
                    parentWrapper.addClass('d-none');
                    otherDetails.removeClass('d-none');
                }
            });
